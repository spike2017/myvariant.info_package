        ...
        if (self.outputtype =='vcf'):
            ...
            for item in queryinfo_list:
                if count < 1000:
                    ...
                    listunit = []
            p = Pool(2)
            results = p.map(target=self.mv.querymany, args=(listall, 'dbsnp.rsid', returnall=True))
            #results = p.map(self.mv.querymany,listall, 'dbsnp.rsid', returnall=True)
            print(type(results))
            print(len(results))
...
demo = query('23andme_large.txt','time_record.txt','23andme','vcf', 19,'None',29,query_count)
demo.genequery()
